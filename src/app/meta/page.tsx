...
export default function EmbeddedSignup() {
  ...
  const launchWhatsAppSignup = useCallback(() => {
    if (!sdkLoaded || typeof window.FB === 'undefined') {
      console.error('Facebook SDK ainda não está disponível. Tente novamente em breve.')
      return
    }
      ...
      try {
        ...
        if (data.type === 'WA_EMBEDDED_SIGNUP') {
          ...
          if (data.event === 'FINISH') {
            ...
            console.log('Sucesso! Phone ID:', phone_number_id, ' | WABA ID:', waba_id)
  ...
  return (
    <div style={{ padding: '20px' }}>
      ...
      >
        {sessionInfo ? JSON.stringify(sessionInfo, null, 2) : 'Aguardando MessageEvent...'}
      </pre>
      ...
      >
        {sdkResponse ? JSON.stringify(sdkResponse, null, 2) : 'Aguardando FB.login response...'}
      </pre>
    </div>
  )
}
